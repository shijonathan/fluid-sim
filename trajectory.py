...
def substep(fs: float):
    ...
    for p in x:
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbor))):
            ...
            if external_grid_forces:
                grid_v[base + offset] += fs * v[p] 
    ...
    for I in ti.grouped(grid_m):
        ...
        grid_v[I] += dt * ti.Vector([0, -9.8, 0]) # Gravity
...
def show_options():
    global external_grid_forces
    with gui.sub_window("Options", 0.05, 0.1, 0.3, 0.15) as w:
        external_grid_forces = w.checkbox("Grid forces", external_grid_forces)
        FORCE_STRENGTH[0] = w.slider_float("Force strength", FORCE_STRENGTH[0], 0, 10)
